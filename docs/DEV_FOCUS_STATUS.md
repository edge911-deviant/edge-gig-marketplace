...
| Firestore authorization tests | Implemented; CI verification pending | `tests/firestore.rules.test.ts` covers identity, profiles, applications, atomic hiring/cancellation, lifecycle, and reviews. Local execution requires Java 21; GitHub CI provides it. |
...
Feature development is on track and the DEV Focus code scope is complete. Production release is not yet green until the GitHub CI run passes and the Firebase console/deployment operations above are completed. The web build also emits a large-bundle warning (~1.08 MB minified); code-splitting is a recommended performance follow-up, not a DEV Focus blocker.
